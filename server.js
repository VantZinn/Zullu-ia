...
app.get("/api/health", (req, res) => {
  res.json({ ok: true, name: "Zulu", company: "V&D Digital", version: "4.0.3", auth: "Supabase" });
});
...
app.post("/api/chats/:id/message", auth, async (req, res) => {
  try {
    ...
    const profile = await getProfile(req.zulu.client, req.zulu.user);
    ...
    const memory = await getMemoryText(req.zulu.client, req.zulu.user.id);
...
`;
    ...
    const { error: insertError } = await req.zulu.client.from("messages").insert([
      { chat_id: chatId, user_id: req.zulu.user.id, role: "user", content: message },
      { chat_id: chatId, user_id: req.zulu.user.id, role: "assistant", content: reply }
    ]);
    ...
    // CORREÇÃO 4.0.3: agora envia explicitamente o UUID da conta.
    extractMemories(req.zulu.client, req.zulu.user.id, chatId, message);
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Zulu V4.0.3 online na porta ${PORT}`);
});
